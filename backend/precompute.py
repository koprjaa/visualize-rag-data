...
def load_id_mapping():
    ...
    folder = find_collection_folder(CHROMA_DB_PATH, "index_metadata.pickle")
    if folder is None:
        raise FileNotFoundError(CHROMA_DB_PATH / "*" / "index_metadata.pickle")

    pickle_file = folder / "index_metadata.pickle"
...
def read_all_embeddings(dimensions=1024):
    ...
    folder = find_collection_folder(CHROMA_DB_PATH, "data_level0.bin")
    if folder is None:
        raise FileNotFoundError(CHROMA_DB_PATH / "*" / "data_level0.bin")

    data_file = folder / "data_level0.bin"
    print(f"Reading embeddings from {data_file}...")

    num_vectors, entry_size, vector_offset = parse_hnsw_header(
        (folder / "header.bin").read_bytes()
    )
    print(f"   Header: {num_vectors} vectors, entry_size={entry_size}, "
          f"vector_offset={vector_offset}")

    data = data_file.read_bytes()
    print(f"   File contains {len(data) // entry_size} vector slots")

    return extract_vectors(data, num_vectors, entry_size, vector_offset, dimensions)
